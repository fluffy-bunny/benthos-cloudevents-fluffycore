...
func (suite *CloudEventsTestSuite) TestExample() {
	...
	for _, ce := range suite.CloudEvents {
		err := kceService.validateCloudEvent(&proto_kafkacloudevent.SubmitCloudEventsRequest{
			Batch: &proto_cloudevents.CloudEventBatch{
				Events: []*proto_cloudevents.CloudEvent{
					&ce.CloudEvent,
				},
			},
		})
